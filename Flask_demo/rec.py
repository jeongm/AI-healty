...
def getNut(uRDI):
    result = []
    result.append(round((uRDI*0.32)/4, 3))
    result.append(round((uRDI*0.32)/4, 3))
    result.append(round((uRDI*0.36)/9, 3))
    
    return result

def getMenu(uRDI, user_id, mode):
    ...
    user_W =  pd.read_sql("SELECT * FROM user WHERE userid=\'"+str(user_id)+"\'",con, index_col='user_seq')
    for i in range(15):
